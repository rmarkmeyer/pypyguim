...
##%EXTRACLASSCODE
     def saveBack(self):
          self.globals.documentation = gettext(self.bigTA)
     
     def setGlobals(self, globals):
          self.globals = globals
          settext(self.bigTA, globals.documentation)
     
     def resizeMe(self, event):
           w, h = event.width, event.height
           self.mycanvas.place(x=0, y=0, width=w, height=h-24)
     ...
     def reconfigure(self, event):
          if event.widget == self.root:
               if event.width != self.lastwidth or event.height != self.lastheight:
                    ...
                    self.bigTA.place(width=self.lastwidth-12, height=self.lastheight-45)
                    y = self.bigTA.winfo_height() + self.bigTA.winfo_y() + 2
                    center = self.lastwidth // 2
                    for component in [self.applyB, self.cancelB]:
                          component.place(y=y)
                    self.applyB.place(x=center-67)
                    self.cancelB.place(x=center)
     
     def post_initialization(self):
          self.lastwidth = 780
          self.lastheight = 474
          self.root.bind("<Configure>", self.reconfigure)
          self.root.title("Documentation window")
##%ENDCODE

     def __init__(self):
          self.root=Tk()
          self.root.geometry("775x430")
...
#--------------------widget making code, do not change anything from here to the end of the file!--------------------
          ...
          self.applyB.place(x=314,y=393, width=67, height=26)
          ...
          self.cancelB.place(x=391,y=393, width=67, height=26)
          ...
          self.bigTA.place(x=10,y=5, width=768, height=384)
          self.bigTA.settext("")
...
##%VERSION=PY2
##%CLASS_STYLE=class
##%WHENWRITTEN=Fri Jun 03 23:07:51 EDT 2022
##%CLASSNAME=DocWindow
##%PACKAGENAME=
##%DIRECTORY=
##%GUITYPE=Python
